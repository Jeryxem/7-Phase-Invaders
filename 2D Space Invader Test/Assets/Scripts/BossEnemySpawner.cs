...
public class BossEnemySpawner : MonoBehaviour
{
    ...
    private void Awake() {
        ...
        maxTopSpawnPoint = GameObject.Find("PointMaxTop-R").transform.position;
        maxBottomSpawnPoint = GameObject.Find("PointMaxBottom-R").transform.position;
        enemyPrefab = enemyPrefabList[0];
        boss = GameObject.Find("EnemyBossTest").GetComponent<Boss>();
    }

    private void Start() {
        boss.canvasManager.playerController.moveSpeed = 0f;
        boss.moveSpeed = 0f;
        boss.canShoot = false;
        boss.canvasManager.playerController.canShoot = false;
        boss.canvasManager.ShowDialogue("GET READY FOR PHASE 1 !");
    ...
    private void NextWave() {
        if(waveLimit > 0) {
            ...
            currentWave += 1;
            enemyPrefab = enemyPrefabList[enemySpawner.sharedRandomRange];
    ...
    public void NextLevel() {
        ...
        if (currentLevel >= 8) { waveLimit = 30; }
    ...
    public void StartSpawning() {
        boss.canvasManager.ShowDialogue("BEGIN!");

        boss.canvasManager.playerController.moveSpeed = 5f;
        boss.moveSpeed = 5f;
        boss.canShoot = true;
        boss.canvasManager.playerController.canShoot = true;
